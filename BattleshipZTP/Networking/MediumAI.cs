...
public class MediumAI : SimpleAI
{
    public override Point GetNextMove(int width, int height, IBattleBoard board)
    {
        ...
        if (largestArea.Count > 0)
        {
            finalTarget = largestArea[_rnd.Next(largestArea.Count)];
        }
        else
        {
            do
            {
                finalTarget = new Point(_rnd.Next(width), _rnd.Next(height));
            } while (ContainsPoint(_alreadyShot, finalTarget) || IsAdjacentToSunkShip(finalTarget, board));
        }

        _alreadyShot.Add(finalTarget);
        return finalTarget;
    }
